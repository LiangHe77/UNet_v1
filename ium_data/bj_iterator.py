...
import sys
sys.path.append("/media/4T/Zyb/TCN_padd_618/")
...
class BJIterator(object):
    ...
    def __init__(self, datetime_set, sample_mode, begin_idx=None, end_idx=None,
                 width=600, height=600, raw_width=800, raw_height=800,
    ...
    def get_frame_paths(self, datetime_batch):
        ...
        for dt in datetime_batch:
            ...
            batch = [os.path.join(unzip_radar_path, dt[0:8], dt[8:]+".nc") for dt in batch]
...
if __name__ == "__main__":
    ...
    begin = time.time()
    for i in range(100):     ## read 100 frame data
        frame_dat, mask_dat, datetime_batch, _ = train_bj_iter.sample(batch_size=4)
        #if datetime_batch[0][0] ==" ":
          # np.save('1.npy',frame_dat)

        print(np.max(frame_dat))
        print(np.min(frame_dat))
        print(np.max(mask_dat))
        print(np.min(mask_dat))
